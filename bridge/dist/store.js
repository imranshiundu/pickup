 ...
 */
export async function getLast(projectDir, processName) {
    ...
    for (let i = all.length - 1; i >= 0; i--) {
        const record = all[i];
        if (projectDir && record.project_dir !== projectDir)
            continue;
        if (processName && record.process_name !== processName)
            continue;
        return record;
    }
    return null;
 ...
 */
export async function listRecords(limit = 10, projectDir) {
    const all = await readStore();
    let filtered = all;
    if (projectDir) {
 ...
 */
export async function appendCheckpoint(checkpoint) {
